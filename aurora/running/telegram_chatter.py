...
from aurora.configuring.tools import suppress_and_log
...
class TelegramChatter(AsyncCallbackHandler):
    def __init__(
        ...
        self.max_visible_ui_len = max_visible_ui_len
    ...
    async def _send_or_edit_message(self):
        ...
        visible_text = accumulated_text[: self.max_visible_ui_len]
        if not visible_text.strip():
            return
        if self.message is None and self.tokens_received >= self.first_token_threshold:
            logger.info("Sending message")
            self.message = await self.original_message.reply_text(visible_text)
            self.last_accumulated_text = visible_text
        else:
            if visible_text != self.last_accumulated_text:
                logger.info("Editing message")
                await self.message.edit_text(visible_text)
                self.last_accumulated_text = visible_text
    ...
    async def _send_typing_action_periodically(self):
        ...
        while (
            not self.done_event.is_set()
            and self.tokens_received < self.first_token_threshold
        ):
